...
double double_copy(double value) {
    return value;
}

void double_delete(double value) {
    (void)value;
}
...
int main() {
    printf("=== DOUBLE VECTOR TESTS ===\n\n");

    Vector vec = create_vector(2, double_copy, double_delete);
    printf("[1] The first vector created: size = %zu, capacity = %zu\n", vec.size, vec.capacity);

    push_back_vector(&vec, 1.1);
    push_back_vector(&vec, 2.2);
    push_back_vector(&vec, 3.3);
    printf("[2] After push_back_vector in the first vector: size = %zu, capacity = %zu\n", vec.size, vec.capacity);

    printf("[3] Elements of the first vector: ");
    for (size_t i = 0; i < vec.size; ++i) {
        printf("%.2f ", get_at_vector(&vec, i));
    }
    printf("\n");

    Vector vec_copy = create_vector(3, double_copy, double_delete);
    push_back_vector(&vec_copy, 1.1);
    push_back_vector(&vec_copy, 2.2);
    push_back_vector(&vec_copy, 3.3);
    ...
    for (size_t i = 0; i < vec_copy.size; ++i) {
        printf("%.2f ", get_at_vector(&vec_copy, i));
    }
    printf("\n");
